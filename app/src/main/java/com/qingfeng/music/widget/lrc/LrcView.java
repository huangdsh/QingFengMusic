package com.qingfeng.music.widget.lrc;
...
public class LrcView extends View implements ILrcView {
    ...
    private int mSeekLineTextSize = sp2px(getContext(), 10);
    private final int mMinSeekLineTextSize = sp2px(getContext(), 8);
    private final int mMaxSeekLineTextSize = sp2px(getContext(),
            13);
    // font size of lrc
    private int mLrcFontSize = sp2px(getContext(), 17);
    private final int mMinLrcFontSize = sp2px(getContext(), 10);
    private final int mMaxLrcFontSize = sp2px(getContext(), 30);
    ...
    private OnLrcSeekChangeListener mOnLrcSeekChangeListener;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mDisplayMode == DISPLAY_MODE_SEEK) {
                    ...
                    // 指定播放的位置（以毫秒为单位）
                    if (mOnLrcSeekChangeListener != null) {
                        mOnLrcSeekChangeListener.onLrcSeekChanged(timeConvert(str));
                    }
                }
    ...
    public interface OnLrcSeekChangeListener {
        void onLrcSeekChanged(int msec);
    }

    public void setOnLrcSeekChangeListener(OnLrcSeekChangeListener onLrcSeekChangeListener) {
        mOnLrcSeekChangeListener = onLrcSeekChangeListener;
    }
}
